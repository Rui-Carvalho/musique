...
(function (angular, window) {
  ...
  disqusModule.provider('$disqus', function() {
     ...
     * @param {String} shortname disqus shortname
     */
    function setGlobals(id, url, title, shortname, payload) {
      window.disqus_identifier = id;
      window.disqus_url        = url;
      window.disqus_shortname  = shortname;
      window.disqus_disable_mobile = true;
      window.disqus_config = function () {
          this.page.title = title;
          this.page.remote_auth_s3 = payload;
          this.page.api_key = window.apiPublicKey;
      };
    }
     ...
     */
    function resetCommit(id, url, title, payload) {
        window.DISQUS.reset({
            reload: true,
            config : function() {
    ...
    this.$get = [ '$location', function($location) {
       ...
       */
      function commit(url, title, payload) {
        var shortname = getShortname(),
            id = url.split('/').slice(-1).pop();
        ...
        if (!angular.isDefined(shortname)) {
          ...
          resetCommit(id, url, title, payload);
        } else {
          setGlobals(id, url, title, shortname, payload);
